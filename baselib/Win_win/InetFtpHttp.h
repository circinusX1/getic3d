...
typedef long (*PDAV)(BYTE* pdata, long dwlength, void* pUser);
...
public:
    BOOL    Connect(const string& user, const string& password, const string& address, const int port=80);
    void    Break(){b_break = 1;};
    BOOL    IsBroke(){return b_break;}
    BOOL    GetFile(const string& fullname, HWND dipMsg=0);
    BOOL    PutFile(const string& filename, const string& strpath);
    void    Callback(const char* strStatus){};
    void    Disconnect();
...
struct Ip
{
    ...
    bool ParseUrl(const tstring& ip)
    {
        ...
        size_t    laststalh = s_target.rfind("/");
        size_t    firststalh = s_target.find("/", 1);
        s_path    = s_target.substr(0,laststalh);
        s_file    = s_target.substr(laststalh);
        return true;
    }
    tstring  s_url;
    tstring  s_user;
