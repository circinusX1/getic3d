...
void Brush::RecalcRotation()
{
    ...
    V3 at = _pUnrotated->_box.GetCenter();
...
void    Brush::ModifyTexCoord(int idx)
{
    Polys::iterator b  = _polys.begin();
    Polys::iterator e  = _polys.end();
    for(;b!=e;b++)
    {
        ...
        else
        {
            ...
            for(;vb!=ve;vb++)
            {
                fv = vb->_xyz;
                fv.norm();
                vb->_uv[idx] = UV(((fv.x/PI)+.50), ((-fv.y/PI)+.50));
                         ...
                         TEX_MAP tm)
{
    ...
    */
    ...
    for(int i=0; i < yDimension; i++)
    {
        if(sThick > 0)
        {
            bt[i].MakeCube(V3(dims.x, sThick, stepZ+(stepZ/8)));
        }
        else
            bt[i].MakeCube(V3(dims.x, stepY*(i+1), stepZ+(stepZ/8)));
...
//--|    Brush::GenTexCoord|--------------------------------------------------------------
void    Brush::ResetTexCoords(int ti)
{
...
void Brush::MakeTerrain(V3& dims, int xDimension, int yDimension, BYTE* pBuffer)
{
    ...
    for(int i=0; i < yDimension; i++)
    {
        ...
        for(int j=0; j < xDimension; j++)
        {
            v[3]._xyz = V3(xs,ys,-GetVal(pBuffer,xDimension,j,i));
            v[2]._xyz = V3(xs+xstp,ys,-GetVal(pBuffer,xDimension,j+1,i));
            v[1]._xyz = V3(xs+xstp,ys+ystp,-GetVal(pBuffer,xDimension,1+j,1+i));
            v[0]._xyz = V3(xs,ys+ystp,-GetVal(pBuffer,xDimension,j,1+i));
            
            v[3]._uv[TX_0] = UV(us, vs);
            v[2]._uv[TX_0] = UV(us+ustp, vs);
            v[1]._uv[TX_0] = UV(us+ustp, vs-vstp);
            v[0]._uv[TX_0] = UV(us, vs-vstp);
...
BOOL  Brush::RecalcBoxAndNormals(BOOL withnormals)
{
	do{
		...
		FOREACH(Polys, _polys, p)
		{
			...
			p->_pBrush = this;
            if(0 == hasalphas)
                this->_brushprops &= ~MODEL_MAT_OPAQUE;
