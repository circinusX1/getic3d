...
//--|     Compiler::Save|-----------------------------------------------------------------
BOOL     Compiler::Save(const char* pszFileName)
{
    REDIR();
    map<int,int>        texids;
    map<int,int>        sndids;
...
void    Compiler::WriteHeader(FlStream& fw)
{
    ...
    strcpy(fh.sig,GeticGeometryBeamTree);
    fh.version  = BSP_VERSION1 ;
    strcpy(fh.comments, "Generated by 'Getic 3D");
...
void Compiler::WriteTexInfo(map<int,int>& texids, FlStream& fw)
{
    ...
    for(;bt != et; bt++)
    {
        ...
        if(strlen(sFullPath) > (MAX_SLEN-4))
        {
        ...
        if(ti.texname[0]=='&')
            strcpy(ti.texname,ti.texname+1);
...
void    Compiler::WriteScripts(FlStream& fw)
{
    ...
    FOREACH(vvector<ScriptItem*>, (*ps) ,ppScr)
    {
        do{
            ...
            strcpy(bsi.scrName ,pSi->_script.c_str());
...
void    Compiler::WriteSndInfo(map<int,int>& sndInfo, FlStream& fw)
{
    ...
    FOREACH(vvector<CSoundItem*>,  (*psnds), ppsnd)
    {
        ...
        strcpy(bsi.sndname,psnd->_soundfile.c_str());
...
void    Compiler::WriteSounds(FlStream& fw)
{
    ...
    FOREACH(vvector<CSoundItem*>,  (*psnds), ppsnd)
    {
        ...
        if(fw2.Open(MKSTR("res\\%s",psnd->_soundfile.c_str()),"rb"))
        {
            ...
            strcpy(sw.sndname,psnd->_soundfile.c_str());
...
void    Compiler::WriteLMInfoAndData(FlStream& fw)
{
    ...
    if(Compiler::PCompiler->_lmConst)
    {
        ...
        vvector<size_t>::iterator itofs = offsets.begin();
#endif //
...
//--|    Compiler::WriteV3s|--------------------------------------------------------------
void    Compiler::WriteV3s(vvector<V3>& vertexPool,FlStream& fw, size_t sect)
{
...
void    Compiler::WritePlanes(vvector<Plane>& allPlanes, FlStream& fw, size_t sect)
{
...
//--|    Compiler::WriteNodes|------------------------------------------------------------
void    Compiler::WriteNodes(vvector<CMiniNode*>& allNodes, FlStream& fw, size_t sect)
{
...
//--|    Compiler::WriteLeafs|------------------------------------------------------------
void    Compiler::WriteLeafs(vvector<CLeaf*>& allLeafs,   FlStream& fw, size_t sect)
{
...
void    Compiler::WriteModels(FlStream& fw, size_t sect)
{
...
void    Compiler::WriteSceneInfo(FlStream& fw)
{
    ...
    if(si.sceneLogoFile[0])
    {
        ...
        if(fw.Open(si.sceneLogoFile,"rb"))
        {
            ...
            ::strcpy((char*)bsect->ptr, si.sceneLogoFile);
            si.sceneSpash       = 0;
        }
    }
...
void    Compiler::WriteItemsInfo(map<int,int>& sndidx, map<int,int>& texids ,FlStream& fw)
{
    ...
    FOREACH(vvector<SceItem*>, (*pItems), ppItem)
    {
        ...
        ::strcpy(btItem._name, pItem->_name);
        ...
        switch(btItem._type)
        {
            ...
            case ITM_STARTPLAY:
            {
                ...
                if(spi->_team.length())
                    strcpy(btItem._startitem._team, spi->_team.c_str());
                else
...
void Compiler::KeepSecttions(size_t nstart, size_t nend, BTF_Section& s, FlStream& fw)
{
...
//--| __MakeZipExtension|-----------------------------------------------------------------
static void  __MakeZipExtension(const char* bspNameFinal, PTSTR bspNameFinalZ, const char* ext)
{
    ::strcpy(bspNameFinalZ, bspNameFinal);
...
//--| _HookCopyFile|----------------------------------------------------------------------
static BOOL  _CompressZip(LPTSTR bspNameFinal, const char* ext)
{
    char	bspNameFinalZ[PATH_MAX];
    ...
    if(ok)
    {
        ...
        ::strcpy(bspNameFinal, bspNameFinalZ);
    }
    return ok;
}

//--| _HookCopyFile|----------------------------------------------------------------------
static size_t _GetFileSize(const char* fName)
{
    size_t fsz = INVALID_FILE_SIZE;
...
static BOOL  _HookCopyFile(const char* scrDir,  const char* fName, 
                           const char* destDir, LPTSTR dstFullName)
{
    char   scrFullName[PATH_MAX];

    ::sprintf(scrFullName, "%s%s", scrDir, fName);
    ::sprintf(dstFullName, "export\\common\\%s", fName);

    char* pqm = strchr(scrFullName,'?'); if(pqm)*pqm=0;
    pqm = strchr(dstFullName,'?'); if(pqm)*pqm=0;
    
    size_t srcDim = _GetFileSize(scrFullName);
    size_t dstDim = _GetFileSize(dstFullName);
    ...
    if(dstDim != INVALID_FILE_SIZE) 
    {
        ...
        {
            ::sprintf(dstFullName, "export\\%s\\%s", destDir, fName);
        }
    }
...
{
    REDIR();
	char	xmlFname[PATH_MAX];
	char	newDirName[PATH_MAX];
	char	bspNameFinal[PATH_MAX];
    char	destLocationFname[PATH_MAX];
    size_t   flength;

    vvector<tstring>     dependencies;
    vvector<size_t>       lengths;
	...
	::sprintf(newDirName,"export\\%s",phBsp.JustFile());
	::mkdir(newDirName);

	::sprintf(bspNameFinal,"%s\\%s",newDirName, phBsp.File());
            ...
            if((*bt).first.find('.') != -1)
            {
                ...
                if(::_HookCopyFile("res\\", ph.File(), phBsp.JustFile(), destLocationFname))
                {
                        ...
                        _CompressZip(destLocationFname,".gtz");
                    {
                        ...
                        char *pStart = _tcsstr(destLocationFname,"export\\")+strlen("export\\");
                        Replace(pStart, '\\', '/');
                        dependencies << pStart;
                        lengths << flength;
                    }
	...
	::sprintf(xmlFname,"export\\%s.ml",phBsp.JustFile());
...
//--|     Compiler::ExportBSP|------------------------------------------------------------
BOOL     Compiler::ExportBSP(const char* pFIleName)
{
    return 1;
}
