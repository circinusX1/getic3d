...
//--| CEditDropTargetImpl : public COleDropTarget|----------------------------------------
...
CCrystalTextView* CCrystalEditView::PView;
...
ON_COMMAND(ID_SC_PASTE, OnEditPaste)
ON_UPDATE_COMMAND_UI(ID_SC_CUT, OnUpdateEditCut)
ON_COMMAND(ID_SC_CUT, OnEditCut)
ON_UPDATE_COMMAND_UI(ID_SC_PASTE, OnUpdateEditPaste)
ON_COMMAND(ID_SC_DELETE, OnEditDelete)
ON_WM_CHAR()
ON_COMMAND(ID_SC_DELETE_BACK, OnEditDeleteBack)
ON_COMMAND(ID_SC_UNTAB, OnEditUntab)
ON_COMMAND(ID_SC_TAB, OnEditTab)
ON_COMMAND(ID_SC_SWITCH_OVRMODE, OnEditSwitchOvrmode)
ON_UPDATE_COMMAND_UI(ID_SC_SWITCH_OVRMODE, OnUpdateEditSwitchOvrmode)
ON_WM_CREATE()
ON_WM_DESTROY()
ON_COMMAND(ID_SC_REPLACE, OnEditReplace)
ON_UPDATE_COMMAND_UI(ID_SC_UNDO, OnUpdateEditUndo)
ON_COMMAND(ID_SC_UNDO, OnEditUndo)
ON_UPDATE_COMMAND_UI(ID_SC_REDO, OnUpdateEditRedo)
ON_COMMAND(ID_SC_REDO, OnEditRedo)
//}}AFX_MSG_MAP
ON_UPDATE_COMMAND_UI(ID_SC_INDICATOR_READ, OnUpdateIndicatorRead)
ON_UPDATE_COMMAND_UI(ID_INDICATOR_OVR, OnUpdateIndicatorOvr)
ON_UPDATE_COMMAND_UI(ID_SC_INDICATOR_COL, OnUpdateIndicatorCol)
END_MESSAGE_MAP()



//--| CCrystalEditView::ResetView|--------------------------------------------------------
...
//--| CCrystalEditView::QueryEditable|----------------------------------------------------
...
//--| CCrystalEditView::DeleteCurrentSelection|-------------------------------------------
...
void CCrystalEditView::OnChar(size_t nChar, size_t nRepCnt, size_t nFlags)
{
    ...
    if (nChar == VK_RETURN)
    {
        ...
        if (QueryEditable() && m_pTextBuffer != NULL)
        {
            ...
            CPoint ptCursorPos = GetCursorPos();
            ASSERT_VALIDTEXTPOS(ptCursorPos);
            const static char pszText[3] = _T("\r\n");
...
//--| CCrystalEditView::OnEditSwitchOvrmode|----------------------------------------------
void CCrystalEditView::OnEditSwitchOvrmode()
{
    m_bOvrMode = ! m_bOvrMode;
}

//--| CCrystalEditView::OnUpdateEditSwitchOvrmode|----------------------------------------
...
//--| CEditDropTargetImpl::OnDragLeave|---------------------------------------------------
...
//--| CCrystalEditView::ShowDropIndicator|------------------------------------------------
...
//--| CCrystalEditView::HideDropIndicator|------------------------------------------------
...
//--| CCrystalEditView::GetDropEffect|----------------------------------------------------
DROPEFFECT CCrystalEditView::GetDropEffect()
{
    return DROPEFFECT_COPY | DROPEFFECT_MOVE;
}

//--| CCrystalEditView::OnDropSource|-----------------------------------------------------
...
//--| CCrystalEditView::OnEditReplace|----------------------------------------------------
...
//--| CCrystalEditView::OnUpdateEditUndo|-------------------------------------------------
...
//--| CCrystalEditView::OnUpdateEditRedo|-------------------------------------------------
    ...
    if (m_bAutoIndent)
    {
        ...
        if (nAction == CE_ACTION_TYPING && _tcscmp(pszText, _T("\r\n")) == 0 && ! m_bOvrMode)
        {
            ...
            if (nPos > 0)
            {
                ...
                char *pszInsertStr = (char *) _alloca(sizeof(char) * (nLength + 1));
